...
def create_image_frame(title, text_lines, width=1080, height=1920, bg_color=(34, 49, 63), text_color=(255, 255, 255)):
    """
    ...
    # Draw Title
    draw.text((width // 2, 300), title, font=title_font, fill=(241, 196, 15), anchor="mm")
    
    # Draw lines of text
    y_offset = 600
    for line in text_lines:
        draw.text((width // 2, y_offset), line, font=body_font, fill=text_color, anchor="mm")
        y_offset += 100
...
def build_kids_video(title, script_paragraphs, output_filename="shorts_output.mp4"):
    ...
    for i in range(total_frames):
        current_time = i / fps
        para_idx = min(int(current_time / duration_per_para), len(script_paragraphs) - 1)
